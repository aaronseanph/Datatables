...
class DTBuilderCollection extends DTBuilderTemplate
{
     ...
     */
    public function __construct(Collection $collectionIn, DTRequest $requestIn, ?string $collectionNameIn = null, $tableName = null)
    {
    ...
    {
        $data = [];
        if ( $this->tableName === 'element' ) {
            foreach ( $this->obj as $key => $obj ) {
                $data[$key] = $obj;
                $data[$key]['group'] = $obj->group;
                $data[$key]['media_attribute'] = $obj->media_attribute;
                $data[$key]['department'] = $obj->department;
                $data[$key]['client'] = $obj->client;
            }
